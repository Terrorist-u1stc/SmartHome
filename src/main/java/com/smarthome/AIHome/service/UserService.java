...
public class UserService {
    ...
    public ApiResponse<User> login(String phoneNumber){
        ApiResponse<User> apiResponse = new ApiResponse<>();
    ...
    public ApiResponse<Void> reset(String newPassword, String phoneNumber) {
        ApiResponse<Void> apiResponse = new ApiResponse<>();
        {
            int rowsAffected = userMapper.updatePassword2(newPassword, phoneNumber);
            if (rowsAffected > 0) {
                apiResponse.setMessage("密码更改成功");
                apiResponse.setCode(200);
            } else {
